...
test_failUpdate_update {
	failUpdate with input as {"stages": {"foo":[{"Cmd": "run", "Value": ["yum update"]}]}}
}

test_failUpdate_update_to {
	failUpdate with input as {"stages": {"foo":[{"Cmd": "run", "Value": ["yum update-to"]}]}}
}

test_failUpdate_upgrade {
	failUpdate with input as {"stages": {"foo":[{"Cmd": "run", "Value": ["yum upgrade"]}]}}
}

test_failUpdate_upgrade_to {
	failUpdate with input as {"stages": {"foo":[{"Cmd": "run", "Value": ["yum upgrade-to"]}]}}
}
