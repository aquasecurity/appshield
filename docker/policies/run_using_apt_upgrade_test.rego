...
test_failUpgrage_basic {
	failUpgrade with input as {"command": {"foo":[{"Cmd": "run", "Value": ["apt-get upgrade"]}]}}
}

test_failUpgrage_dist {
	failUpgrade with input as {"command": {"foo":[{"Cmd": "run", "Value": ["apt-get dist-upgrade"]}]}}
}

test_entry_point_positive {
	r := deny with input as {"command": {"foo":[{"Cmd": "run", "Value": ["apt-get dist-upgrade"]}]}}
