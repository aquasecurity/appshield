...
test_failAdd_basic {
	not failAdd with input as  {"stages": {"foo":[{"Cmd": "run", "Value": ["tar -xjf /temp/package.file.tar.gz"]}]}}
}

test_failAdd_copy {
	not failAdd with input as {"stages": {"foo":[{"Cmd": "copy", "Value": ["test.txt", "test2.txt"]}]}}
}

test_failAdd_add {
	failAdd with input as {"stages": {"foo":[{"Cmd": "add", "Value": ["/target/app.jar", "app.jar"]}]}}
}

test_failAdd_many_add {
	failAdd with input as {"stages": {"foo":[
...
test_failAdd_add_archive {
	not failAdd with input as {"stages": {"foo":[{"Cmd": "add", "Value": ["/target/resources.tar.gz", "resources.jar"]}]}}
}
